...
function MoviesContextProvider({children}){
    ...
    const URLMovies = 'https://backend-movies-page.vercel.app'
    ...
    effectRef.current = async ()=>{
        try {
            const data = await getData(`${URLMovies}/movies`)
        ...
        if(!inputTitle || !inputYear || !inputURL || !inputDescription){
            ...
            try{
                ...
                if(!(isImage && yearIsNumber)){
                 ...
                 postData(`${URLMovies}/movies`, newMovie);
                 cleanData()
                 ReloadMovies()
                 setModalAddMovie(false)
    ...
    const deleteMovie = ()=>{
        deleteData(`${URLMovies}/movies/${modalInfoDescription.id}`)
        ReloadMovies()
        setModalDescriptionOpen(false)
    }
    const openModify = ()=>{
    ...
    const modificarMovie =()=>{
        try{
            ...
            if(inputURLModifying===""){
                ...
                putData(`${URLMovies}/movies`, newMovie);
            }else{
